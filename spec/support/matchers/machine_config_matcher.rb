...
RSpec::Matchers.define :be_a_valid_machine_state_hash do
  match do |actual|
    ...
    if actual.is_a?(Hash)
      ...
      @errors.push("expected reflector state to be present") if refl.nil?
      ...
      pb = actual[:plugboard]
      @errors.push("expected plugboard state to be present") if pb.nil?
